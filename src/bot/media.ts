...
export class MediaPlayer {
    ...
    dispatchStream(stream: Readable, item: MediaItem) {
        ...
        this.dispatcher.on('start', () => {
            ...
            this.determineStatus();
            if(this.channel)
                this.channel.send(`:musical_note: Tocando: "${item.name}", Requested by: ${item.requestor}`);
        });
    ...
    play() {
        ...
        if(this.playing && !this.paused)
            this.channel.send(`Já tocando a música!`);
        ...
        if(item) {
            if(this.playing) {
                if(this.paused) {
                    ...
                    this.status.setBanner(`Tocando: "${item.name}" Requested by: ${item.requestor}${this.queue.length > 1 ? `, Up Next "${this.queue[1].name}"`:''}`);
                }
            } else
                this.status.setBanner(`Asseguir: "${item.name}" Requested by: ${item.requestor}`);
        } else
