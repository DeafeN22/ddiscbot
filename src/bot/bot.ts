        ...
        this.commands = new CommandMap()
            ...
            .on('join', (cmd: ParsedMessage, msg: Message) => {
                joinUserChannel(msg)
                    .then(connection => {
                        ...
                        msg.channel.send(`:speaking_head: Entrou no canal: ${connection.channel.name}`);
            ...
            .on('leave', (cmd: ParsedMessage, msg: Message) => {
                ...
                this.client.voiceConnections.forEach(conn => {
                    conn.disconnect();
                    msg.channel.send(`:mute: Disconectou-se do canal: ${conn.channel.name}`);
                });
            })
                    ...
                    if(!this.player.connection) {
                        joinUserChannel(msg)
                            .then(conn => {
                                ...
                                msg.channel.send(`:speaking_head: Entrou no canal: ${conn.channel.name}`);
                                done();
                            });
                    } else
                        done();
